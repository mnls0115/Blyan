 ...
 */
const apiAuthV2 = new APIAuthManagerV2();

// Export for use in other modules
window.apiAuthV2 = apiAuthV2;
...
window.makeAuthenticatedRequest = (url, options) => {
    return apiAuthV2.makeAuthenticatedRequest(url, options);
};

window.getAuthStatus = () => {
    return apiAuthV2.getAuthStatus();
};

window.requireAuth = () => {
    if (!apiAuthV2.hasValidApiKey()) {
        throw new AuthenticationError('Authentication required', 'auth_required');
    }
};

window.requireScope = (scope) => {
    if (!apiAuthV2.hasScope(scope)) {
        throw new AuthenticationError(
            `Missing required scope: ${scope}`,
            'insufficient_scope',
            { required: scope, available: apiAuthV2.state.keyInfo?.scopes || [] }
        );
    }
};
