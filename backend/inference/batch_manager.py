...
async def demo_batch_manager():
    """Demonstrate batch manager functionality."""
    ...
    model = MockModel()
    engine = BatchedInferenceEngine(model, config)
    ...
    try:
        ...
        # Burst of similar requests (should batch well)
        tasks = []
        for i in range(10):
            task = engine.infer(f"Short prompt {i}", max_new_tokens=32)
            tasks.append(asyncio.create_task(task))
            
        # Different length requests
        for i in range(5):
            long_prompt = " ".join([f"word{j}" for j in range(i * 10)])
            task = engine.infer(long_prompt, max_new_tokens=64)
            tasks.append(asyncio.create_task(task))
            
        # Repeated requests (should hit cache)
        for i in range(3):
            task = engine.infer("Cached prompt", max_new_tokens=32)
            tasks.append(asyncio.create_task(task))
        ...
        # Show statistics
        stats = engine.batch_manager.get_stats()
        print("\n📊 Batch Manager Statistics:")
        print(f"  Total batches: {stats['total_batches']}")
        print(f"  Average batch size: {stats['average_batch_size']:.1f}")
        print(f"  Average wait time: {stats['average_wait_time_ms']:.1f}ms")
