...
# Demo usage
async def demo_dual_model():
    """Demonstrate dual model execution."""
    import torch.nn as nn
    import torch.optim as optim
    import asyncio
    
    print("🚀 Dual Model Manager Demo")
    print("=" * 50)
    ...
    class SimpleModel(nn.Module):
        ...
        def forward(self, input_ids, **kwargs):
            x = self.relu(self.fc1(input_ids))
            x = self.relu(self.fc2(x))
            logits = self.fc3(x)
            
            # Simple loss for demo
            if "labels" in kwargs:
                loss = nn.functional.cross_entropy(logits, kwargs["labels"])
                return type('Output', (), {'loss': loss, 'logits': logits})()
            return logits
    
    # Initialize dual model manager
    base_model = SimpleModel()
    manager = DualModelManager(base_model)
    
    # Setup optimizer for learning
    optimizer = optim.Adam(manager.learning_model.parameters(), lr=1e-3)
    
    print(f"\n📊 Initial memory usage: {manager.get_memory_usage()}")
    
    # Simulate concurrent workload
    async def learning_task():
        """Simulate learning on dedicated stream."""
        print("\n📚 Starting learning task...")
        for i in range(10):
            batch = {
            ...
            result = await manager.learning_step(batch, optimizer)
            
            if i % 3 == 0:
                print(f"  Learning step {i}: loss={result['loss']:.4f}")
    ...
    async def inference_task():
        """Simulate inference on dedicated stream."""
        ...
        for i in range(20):
            ...
            result = await manager.inference(input_data)
            latencies.append(result["inference_time_ms"])
            
            if i % 5 == 0:
                print(f"  Inference {i}: time={result['inference_time_ms']:.1f}ms")
                
            await asyncio.sleep(0.05)  # Higher frequency than learning
            
        avg_latency = sum(latencies) / len(latencies)
        print(f"✓ Inference task completed - avg latency: {avg_latency:.1f}ms")
        
    # Run both tasks concurrently
    print("\n🔄 Running learning and inference concurrently...")
    ...
    print(f"\n💾 Final memory usage: {manager.get_memory_usage()}")
    
    # Test model synchronization
    print("\n🔄 Testing model synchronization...")
    manager.synchronize_models(learning_to_inference=True)
    print("✓ Models synchronized")

if __name__ == "__main__":
    asyncio.run(demo_dual_model())