...
# Demo
async def demo_micro_step_training():
    """Demonstrate micro-step training."""
    ...
    # Simulate training with interruptions
    trainer = MicroStepTrainer(model, optimizer)
    ...
    async def train_task():
        ...
        for batch in dataset:
            result = await trainer.train_step(batch, step)
    ...
    async def inference_simulation():
        for i in range(5):
            await asyncio.sleep(0.5)
            print(f"🚨 Inference request {i} - requesting yield")
            trainer.request_yield(priority=0.8)
    ...
    print("\nFinal metrics:", trainer.get_metrics())

if __name__ == "__main__":
    asyncio.run(demo_micro_step_training())